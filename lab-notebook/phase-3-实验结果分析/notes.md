...
## v13：Conv/FC 分类型张量并行同步原语 + YOLOv5 模型 (2026-03-17)

### 背景

新增 YOLOv5 模型数据集（324 层，104 Conv2d + 101 BN + 101 SiLU + 13 Concat + 3 MaxPool + 2 Upsample）。初次运行发现 **Ours (11910ms) > MEDIA (11432ms)**，HPA 在 YOLOv5 上反而更慢。

### 根因分析

YOLOv5 的所有计算密集型算子均为 Conv2d。卷积层的张量并行策略与全连接层本质不同：

| | FC 层（列并行） | Conv 层（滤波器并行） |
|---|---|---|
| **切分维度** | 输出维度（每个 shard 算部分和） | 输出通道（每个 shard 算独立通道） |
| **通信原语** | **AllReduce**（求和+广播） | **AllGather**（拼接） |
| **通信量** | `2(k-1)/k × output_bytes` | `(k-1)/k × output_bytes` |
| **比值** | 2× | **1×**（半价） |

之前 `hpa_cost` 统一使用 AllReduce，对 Conv 层高估了一倍通信代价 → 很多 Conv 算子无法通过 cost-benefit 筛选。

### 修改方案

1. **`common.py`**：新增 `is_conv_layer(layer)` 函数，优先根据 `layer_type` 字段判断（Conv2d→Conv, Linear/MatMul→FC），回退到层名启发式（`3x3`/`1x1`/`conv`→Conv, `fc`/`proj`/`ffn`→FC）
2. **`common.py`**：`hpa_cost` 同步代价分支——Conv 用 AllGather `(k-1)/k`，FC 用 AllReduce `2(k-1)/k`
3. **`loader.py`**：读取 CSV 的 `type` 列，传入 DNNLayer 的新字段 `layer_type`
4. **`alg_ours.py`**：`_augment_graph` 中同步 barrier 通信量同步区分，barrier 节点命名 `_allgather`/`_allreduce`

### 实验结果（4×Xeon_IceLake, 100Mbps）

| Model | OCC | DINA | MEDIA | Ours (v12) | Ours (v13) | v13 改善 |
|-------|-----|------|-------|------------|------------|---------|
| YOLOv5 | 11613.1 | 14713.5 | 11432.8 | 11910.7 | **8304.4** | **-30.3%** |
| InceptionV3 | 1514.1 | 1611.0 | 1403.1 | 929.8 | **890.7** | -4.2% |
| VGG-16 | 3079.1 | 5886.7 | 2957.1 | 2031.5 | **1353.4** | -33.4% |
| BERT-large | 3040.9 | 14429.0 | 2306.7 | 2287.1 | 2287.1 | 0% |

- **YOLOv5**：候选算子 10→42，AllGather 使大量 Conv 通过筛选
- **VGG-16**：候选算子 6→12（新增 conv1_2, conv3_2, conv3_3, conv4_1, conv4_2, conv4_3）
- **InceptionV3**：候选算子 17→30（名称启发式 `_3x3`/`_1x1` 正确识别 Conv）
- **BERT-large**：无变化（全部 FC/MatMul 层，AllReduce 不变）

### 关键发现

1. **Conv 层 AllGather 通信量仅为 AllReduce 的一半**，使得 conv-heavy 模型（YOLOv5, VGG-16）的张量并行收益大幅提升
2. **YOLOv5** 是继 InceptionV3 之后第二个展示 Ours 显著优势的模型——但原因不同：InceptionV3 靠 DAG 并行分支，YOLOv5 靠 Conv 层的低通信同步
3. VGG-16 的改善（-33.4%）甚至超过 YOLOv5，因为 conv1_2 (543.8ms) 极为计算密集，之前因 AllReduce 代价过高被排除

> **素材标注**：
> - **论文 Design 章节**：HPA 的类型感知同步原语选择——Conv 用 AllGather（滤波器并行），FC 用 AllReduce（列并行），理论通信量差 2×
> - **论文 Evaluation**：YOLOv5 作为 CNN 检测模型的代表，展示 Ours 在非分类任务上的泛化能力；VGG-16 的 conv1_2 是类型感知优化的典型案例
