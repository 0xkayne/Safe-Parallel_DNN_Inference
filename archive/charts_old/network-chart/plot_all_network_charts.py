...
def create_network_chart(csv_file, model_name):
    ...
    linestyles = {
        ...
        'MEDIA': (0, (1, 1)), # Dense dotted (1 on, 1 off)
        'Ours': '-'           # Solid
    }
    ...
    linewidths = {
        ...
        'MEDIA': 2.0,      # Thinner line
        'Ours': 2.5
    }
    
    # Calculate dynamic offset based on data range (using log scale for better visibility)
    # data_min = df[['OCC', 'DINA', 'MEDIA', 'Ours']].min().min()
    # data_max = df[['OCC', 'DINA', 'MEDIA', 'Ours']].max().max()
    ...
    for algo in ['OCC', 'DINA', 'MEDIA', 'Ours']:
        y_data = df[algo].copy()
        # Apply multiplicative offset for DINA and MEDIA
        if algo == 'DINA':
